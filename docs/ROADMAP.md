...
| Stage | Gate |
| --- | --- |
| `1.14.0-beta.1` | B1, B2 and B3 fixed (B4, B5, B6 and B8 already done in `1.13.2-alpha`). Dead code deleted. CI running the suite on PHP 8.4 and 8.5. PHPStan raised from its level-1 floor to at least level 5, green. `composer audit` clean. |
